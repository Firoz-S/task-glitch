...
export function sortTasks(tasks: ReadonlyArray<DerivedTask>): DerivedTask[] {
  return [...tasks].sort((a, b) => {
    ...
    // ✅ Deterministic tie-breaker (no flicker)
...
export function computeVelocityByPriority(
  ...
  (Object.keys(groups) as Task["priority"][]).forEach((k) => {
    ...
    result[k] = { avgDays: avg, medianDays: median };
  });
...
export function computeThroughputByWeek(
  ...
  tasks.forEach((t) => {
    ...
    const entry = map.get(week) ?? { count: 0, revenue: 0 };
    entry.count += 1;
...
export function computeForecast(
  ...
  for (let i = 1; i <= horizonWeeks; i++) {
    result.push({
      week: `+${i}`,
      revenue: Math.max(0, slope * (lastIndex + i) + intercept),
    });
  }
